...
int main() {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        usleep(1'000'000 / 60 -  std::chrono::duration_cast<std::chrono::microseconds>(elapsed).count());
        ...
        ++iteration;
        if(iteration % 60 == 0) {
            float felapsed = std::chrono::duration_cast<std::chrono::microseconds>(elapsed2).count() / 1'000'000.f;
            if(felapsed > 0.f) {
