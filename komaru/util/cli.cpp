...
CLICommandResult::CLICommandResult(int code, std::string sout, std::string serr)
    : code_(code), sout_(std::move(sout)), serr_(std::move(serr)) {
}
...
CLICommandResult PerformCLICommand(const std::string& command) {
    ...
    const std::string final_command = std::format("{} 1> {} 2> {}", command, stdout_path, stderr_path);

    auto pid = fork();

    if (pid < 0) {
        throw std::runtime_error(std::format("Failed to fork when trying to execute \"{}\"", command));
    }
    ...
    if (ret == pid) {
        if (WIFEXITED(status)) {
            ...
            if(!maybe_sout) {
                throw std::runtime_error(std::format("error while reading stdout file \"{}\", error: {}", stdout_path, maybe_sout.error().message()));
            }
            if(!maybe_serr) {
                throw std::runtime_error(std::format("error while reading stdout file \"{}\", error: {}", stderr_path, maybe_serr.error().message()));
            }
            std::filesystem::remove(stdout_path);
            std::filesystem::remove(stderr_path);

            return CLICommandResult(code, std::move(maybe_sout.value()), std::move(maybe_serr.value()));
        } else {
    ...
    kill(pid, SIGKILL);
    throw std::runtime_error(std::format("Failed to wait for child when executing command \"{}\"", command));
}
